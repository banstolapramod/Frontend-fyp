...
export default function CategoryPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`http://localhost:5001/api/products/public/by-category/${categoryId}`)
      .then(r => r.json())
      .then(data => {
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#111 0%,#374151 100%)', padding: '48px 24px 40px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <h1 style={{ color: '#fff', fontSize: 32, fontWeight: 800, margin: '0 0 8px' }}>{categoryName}</h1>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <p style={{ color: '#6b7280', margin: '0 0 24px' }}>No products in the {categoryName} category yet.</p>
            <button onClick={() => navigate('/')} style={{ padding: '12px 28px', background: '#111', color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
              Browse All Products
            </button>
          </div>
        ) : (
